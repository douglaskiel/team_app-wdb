...
!function(){"use strict";function ProductsListController(ProductsService){var vm=this;vm.products=ProductsService.query()}angular.module("products").controller("ProductsListController",ProductsListController),ProductsListController.$inject=["ProductsService"]}();
!function(){"use strict";function ProductsController($scope,$state,$window,Authentication,product){function remove(){$window.confirm("Are you sure you want to delete?")&&vm.product.$remove($state.go("products.list"))}function addMaterial(isValid){vm.productList.push({material:vm.product.material,priority:vm.product.priority,isChecked:!1}),vm.product.material="",vm.product.priority=""}function save(isValid){return vm.product.materials=vm.productList,isValid?void(vm.product._id?vm.product.$update().then($state.go("products.list")):vm.product.$save().then($state.go("products.list"))):($scope.$broadcast("show-errors-check-validity","vm.form.productForm"),!1)}var vm=this;vm.authentication=Authentication,vm.product=product,vm.error=null,vm.form={},vm.remove=remove,vm.save=save,vm.productList=[],vm.addMaterial=addMaterial}angular.module("products").controller("ProductsController",ProductsController),ProductsController.$inject=["$scope","$state","$window","Authentication","productResolve"]}();
